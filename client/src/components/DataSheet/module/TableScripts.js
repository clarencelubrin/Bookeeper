...
*/
export function tableScripts(sheet, input_table_data, table_index, data){
    let table_data = [];
    let table_style = [];
...
function rowScripts(sheet, input_row_data, row_index, prev_row, data) {
    ...
    input_row_data.map((cell, cell_index) => {
        ...
        let new_value = out.value;
        let new_style = out.style;
        row_style.push(new_style);                  // Push new style to row_style
        row_data.push([column_title, new_value]);   // Push new value to row_data
    });
    return { row_data, row_style };
}

function cellScript(sheet, column_title, value, row_index, row_data, data, prev_row){
    let style = ''; // initialize style

    const row_data_dict = listToDict(row_data); // {column_title: cell_value}
    ...
    if (sheet === 'General Journal') {
        if (column_title.toLowerCase() === 'description'){
            ...
            if (creditCell && creditCell !== '' && creditCell !== ' ' && value[0] !== '\t') {
                value = '\t' + value;
            } 
            else {
                value = value.replace('\t', '');
        ...
        if (column_title.toLowerCase() === 'p/r') {
            // Auto P/R
            const description = row_data_dict['DESCRIPTION'];
            value = autoPR(description, data);
        }
    }
    ...
    for (let i = 0; i < chartOfAccountTables.length; i++) {
        for (let j = 0; j < chartOfAccountTables[i].length; j++) {
            ...
            if (key.toLowerCase() === description.toLowerCase()) {
                console.log(chartOfAccountTables[i][j][key]);

...
function DataTableRow({ row_data_values, row_style, row_index }) {
    const { setCheckedRows, checked_rows } = useContext(FunctionContext);
    const { sheet, inputsRef, table_data } = useContext(PropContext);
    const data = useSelector(state => state.data)['content'];
    // Handle hover events
...
function DataTableSubtitle({table_index}) {
    ...
    return (
    <TableSubtitle>
        <TableInput input_type="input" key={0} cell_index={0} row_index={0} column={'Title'}
            value={table_data[0]['Title']} placeholder="Title" 
            handleChange={(e) => {
                updateCell(0, 'Title', e.target.value);
            }}
            inputClass="b-2 pl-1 pr-1 text-center sm:w-3/4 w-full bg-transparent justify-self-center placeholder:font-normal sm:col-span-2 col-span-1"/>
        <p className="text-center">ACCOUNT NO.</p>
        <TableInput input_type="input" key={1} cell_index={1} row_index={0} column={'Account No.'} 
